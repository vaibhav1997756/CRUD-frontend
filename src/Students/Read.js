...
// const Read = () => {
//   const { id } = useParams();
//   const [usercrud, setCrud] = useState(0); // Initialize as null for conditional rendering
  
//   useEffect(() => {
//   axios.get(`http://localhost:8080/read/${id}`)
//       .then(res => {
//         console.log(res);
//         setCrud(res.data); // Assuming res.data is the user object
//       })
//       .catch(err => console.log(err));
...
//         {usercrud ? (
//           <>
//             <h3>ID: {usercrud[id].id}</h3>
//             <h3>Name: {usercrud[id].name}</h3>
//             <h3>Email: {usercrud[id].email}</h3>
//           </>
//         ) : (
...
const Read = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:8080/read/`+id)
      .then(res => {
        console.log(res);
        setCrud(res.data[0]); // Assuming res.data is the user object
      })
      .catch(err => console.log(err));
  }, [id]); // Add 'id' as a dependency for the effect
